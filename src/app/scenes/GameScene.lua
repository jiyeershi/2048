...
function GameScene:ctor()
    ...
    self.player:setPosition(200, display.height * 2 / 3)
...
function GameScene:playerFlyToScene()
    ...
    local function startDrop()
        ...
        print("11166")
        self.backgroundLayer:onTouch( function(event)
            return self:onTouchHandle(event.name, event.x, event.y)
        end)
        self.backgroundLayer:setTouchEnabled(true)
        -- cc.EventListenerTouchOneByOne:create():registerScriptHandler()
    end
...
function GameScene:onTouchHandle(event, x, y)
    if event == "began" then
